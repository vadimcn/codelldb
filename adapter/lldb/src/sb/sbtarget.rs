...
impl SBTarget {
    ...
    pub fn watch_address(&self, addr: Address, size: usize, read: bool, write: bool) -> Result<SBWatchpoint, SBError> {
        ...
        let wp = cpp!(unsafe [self as "SBTarget*", addr as "addr_t", size as "size_t",
            ...
            // The WatchAddress API is a wrapper for WatchpointCreateByAddress, but it's bugged and
            // ignores what you put in 'modify', meaning it always stops even for read-only breakpoing requests.
            // Fortunately, the implementation is trivial, so we can just call WatchpointCreateByAddress directly.
    ...
    pub fn resolve_symbol_context_for_address(&self, addr: &SBAddress, scope: SymbolContextScope) -> SBSymbolContext {
